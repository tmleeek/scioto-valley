...
IWD.OPC = {
		...
		/** SAVE PAYMENT **/		
			...
			if (payment.currentMethod != 'stripe') {
                ...
                //update for PAYPAL LIGHTBOX EXPRESS
...
IWD.OPC.Checkout = {
		...
		prepareAddressResponse: function(response){
			...
			if (typeof(response.shipping) != "undefined"){
				...
				IWD.OPC.Checkout.getAdditional();
			}
		...
		getAdditional: function(){
		    new Ajax.Updater('onepage-checkout-shipping-method-additional-load', IWD.OPC.Checkout.config.baseUrl + 'onepage/json/getAdditional', {onSuccess: function() {
...
IWD.OPC.Decorator = {
		...
		setActivePayment: function(){
			...
			$j_opc(document).on('click','#co-payment-form dt', function(){
				$j_opc('#co-payment-form dt').removeClass('active');
