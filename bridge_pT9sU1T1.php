...
if(!defined('USER_DB_SERVER') || !defined('USER_DB_SERVER_USERNAME') || !defined('USER_DB_SERVER_PASSWORD') || !defined('USER_DB_DATABASE')) {
	...
	elseif ($g_iCartType == 8) {
		...
		define('DB_SERVER_USERNAME', $VMConfig->user);
		define('DB_SERVER_PASSWORD', $VMConfig->password);
		define('DB_TABLE_PREFIX', $VMConfig->dbprefix."mijoshop_");
		$g_sCartHost = $_SERVER['SERVER_NAME'];
	}
}
ob_end_clean();
...
if(md5($username.$password) != $_REQUEST["hash"]) {
	if(md5(DB_SERVER_USERNAME.DB_SERVER_PASSWORD) != $_REQUEST["hash"]){
...
if(isset($_REQUEST["vars_names"])) $vars_names = explode(',', $_REQUEST["vars_names"]);
if(isset($_REQUEST["vars_main_dir"])) $vars_main_dir = $_REQUEST["vars_main_dir"];
if(isset($_REQUEST["xml_path"])) $xml_path = $_REQUEST["xml_path"];
if(isset($_REQUEST["xml_fields"])) $xml_fields = $_REQUEST["xml_fields"];
if(isset($_REQUEST["xml_items_node"])) $xml_items_node = $_REQUEST["xml_items_node"];
if(isset($_REQUEST["xml_items_info_node"])) $xml_items_info_node = $_REQUEST["xml_items_info_node"];
if(isset($_REQUEST["xml_filters"])) $xml_filters = $_REQUEST["xml_filters"];

// for get_ftp_files - Andriy Prudyus
if (isset($_REQUEST["search_path"])) $search_path = $_REQUEST["search_path"];
if (isset($_REQUEST["mask"])) $mask = $_REQUEST["mask"];
if (isset($_REQUEST["ignore_dir"])) $ignore_dir = $_REQUEST["ignore_dir"];

// for put_sql - Andriy Prudyus
...
function handle_dirs($vars_main_dir, $vars_names)
{
    ...
    $itemHandler = opendir(getcwd().'/'.$vars_main_dir);
...
function getCartType() {
	...
	// MijoShop
	if( file_exists(dirname(__FILE__) . '/components/com_mijoshop/opencart/config.php') ) {
		return 8;
	}
	// VirtueMart
...
function get_config() {
	$sConf = array(
		...
		'database_table_prefix' => (defined('USER_TABLE_PREFIX') ? USER_TABLE_PREFIX : (defined('DB_TABLE_PREFIX') ? DB_TABLE_PREFIX : 'xcart_')),
...
function put_sql($import_handle) {
	global $db_link, $import_handle, $sql, $allow_compression, $temporary_dir, $errors, $delete_tmpfile_after_post, $f_name;
	...
	*/
	function _connect() {
		...
		if (!$is_reconnect) {
			...
			if(!$this->rLink) { // check for connection
				$str = sprintf("Error connecting to database");
				return $str;
			} else {
				$this->_report('Selecting database', 30);
				if(!(mysql_select_db($this->sDBName, $this->rLink))) {
					$str = sprintf("Error selecting database");
					return $str;
				}
			}
			return 1;
...
function reconnect() {
    global $cur_count_attempt_post, $max_count_attempt_post, $db_link;
    $GLOBALS['is_reconnect'] = TRUE;
    $mySQL = new cMySQLBackUp();
    for ($i = $cur_count_attempt_post; $i < $max_count_attempt_post; $i++) {
        sleep(20);
        $answer = $mySQL->_connect();
        if ($answer == 1) {
            $db_link = $mySQL->rLink;
            ob_clean();
            return 1;
...
function PMA_importRunQuery($sql = '', $full = '') {
    global $db_link,$import_run_buffer, $go_sql, $complete_query, $display_query, $sql_query, $my_die, $error, $reload, $finished, $timeout_passed, $skip_queries, $executed_queries, $max_sql_len, $read_multiply, $cfg, $sql_query_disabled, $db, $run_query, $is_superuser, $message, $show_error_header, $file_name, $checksum_sm, $prev_sql_exec;
    $run_query = true;
    $read_multiply = 1;
                    ...
                    if($run_query) {
                        ...
                        if(!$result) {
                            ...
                            if (isset($checksum_sm)) {
                                if ((mysql_errno($db_link) == 2006) || (mysql_errno($db_link) == 1317)) {	// Error Code: 2006 - MySQL server has gone away; Error Code: 1317 - Query execution was interrupted
                                    if (reconnect() == 1) {
                                        $result = mysql_query($import_run_buffer['sql'],$db_link);
                                        if (!$result) {
                                            $st = str_replace(';',' ',mysql_error($db_link));
                                            $ret .= POST_ERROR_SQL_INDEX . "|" . $cur_sql_index . '|<font color="#000000"><b>'.mysql_errno($db_link).'; '.$st.'</b></font>;'.htmlspecialchars($import_run_buffer['sql']).'<br>';
                                        ...
                                        $ret .= POST_ERROR_SQL_INDEX . "|" . $cur_sql_index . '|<font color="#000000"><b>'.$st.'</b></font>;'.htmlspecialchars($import_run_buffer['sql']).'<br>';
                                        $fp = fopen($file_name, "w");
                                        if ($fp) {
                                            fputs($fp, $checksum_sm . "|" . --$cur_sql_index);
                                            fclose($fp);
...
function running($import_handle) {
    global $finished, $checksum_sm, $errors, $checksum_prev, $file_name, $prev_sql_exec, $f_name;
    $buffer = $ret = '';
    $file_name = "tmp/sm_tmp.txt";
    $checksum_prev = '';
	...
	if ($fp = opendir($path)) {
		while ($value = readdir($fp)) {
			if (is_file($path . "/" . $value)) {
				if (glob("$path/$mask")) {
					$all_files[] = $path . "/" . $value;		
				}
			}
			elseif (!(in_array($value, $skip)) && is_dir($path . "/" . $value) && !(in_array($path . "/" . $value, $arr_ignore_dir))) {
					get_ftp_files($path . "/" . $value, $mask, $ignore_dir);
			}
		}
		closedir($fp);
	}
