...
class AW_Pquestion2_AnswerController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _validate($answerModel)
    {
        ...
        if (!is_string($content) || strlen($content) <= 0) {
            throw new Exception(
                Mage::helper('aw_pq2')->__("Answer doesn't specified")
            );
        }
        ...
        if (!$questionModel->getId()) {
            throw new Exception(
                Mage::helper('aw_pq2')->__("Can't found the question")
            );
        }
    }
    ...
    public function addAction()
    {
        ...
        if (Mage::helper('aw_pq2/config')->getRequireModerateCustomerAnswer()) {
            if ($isSubscribed) {
                if ($answerModel->getCustomerId()) {
                    $this->_getSession()->addSuccess(
                        $this->__('Your answer has been received. We will notify you when answer status will'
                            . ' be changed. You can track all your questions and its answers <a href="%s">here</a>',
                    ...
                    $this->_getSession()->addSuccess(
                        $this->__(
                            'Your answer has been received. We will notify you when answer status will be changed.'
                        )
                    );
                }
            } else {
                if ($answerModel->getCustomerId()) {
                    $this->_getSession()->addSuccess(
                        $this->__('Your answer has been received.'
                            . ' You can track all your questions and its answers <a href="%s">here</a>',
                            Mage::getUrl('aw_pq2/customer/index',
                                array('_secure' => Mage::app()->getStore(true)->isCurrentlySecure())
                            )
                        )
            ...
            if ($answerModel->getCustomerId()) {
                ...
                $this->_getSession()->addSuccess($this->__('Answer successfully added'));
            }
        }
        return $this->_redirectUrl($this->_getRefererUrl());
    }
