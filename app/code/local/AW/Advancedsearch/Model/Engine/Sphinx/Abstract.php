...
class AW_Advancedsearch_Model_Engine_Sphinx_Abstract
{
    ...
    public function reindex($indexer = null, $delta = false)
    {
        ...
        if ($indexer) {
            if ($this->_createFullIndexConfig()) {
                ...
                ob_start();
                passthru(
                    $path . AW_Advancedsearch_Model_Engine_Sphinx::INDEXER_CALL
                    . ' -c ' . $this->getSphinxEngine()->getConfigFileName() . ' ' . $indexName,
                    $_ret
                );
    ...
    public function mergeDeltaWithMain($indexer = null)
    {
        ...
        if ($indexer) {
            ...
            ob_start();
            passthru(
                $path . AW_Advancedsearch_Model_Engine_Sphinx::INDEXER_CALL . ' -c '
                . $this->getSphinxEngine()->getConfigFileName() . ' --merge '
                . $mainIndex . ' ' . $deltaIndex . ' --rotate',
                $_ret
            );
            $_out = ob_get_contents();
            ob_end_clean();
    ...
    public function getConfigFileContent()
    {
        ...
        $_files = array(
            'index_path'       => $this->getSelfVarDir() . DS . $indexName,
            'delta_index_path' => $this->getSelfVarDir() . DS . $deltaIndexName,
        );

        $morphology = 'none';
        if (Mage::helper('awadvancedsearch/config')->isMorphologyEnabled()) {
            $morphology = Mage::helper('awadvancedsearch/config')->getMorphologyDictionaryList();
        }

        $additionalConfigPart = $sphinx->isNewerThan220()
            ? ''
            : 'charset_type = utf-8' . PHP_EOL . 'enable_star = 1';
...
source {$indexName} : dbconnect
{
    ...
    sql_attr_uint = _product_id
    sql_attr_uint = _store_id
