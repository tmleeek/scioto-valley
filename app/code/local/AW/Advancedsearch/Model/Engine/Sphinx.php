...
class AW_Advancedsearch_Model_Engine_Sphinx
{
    ...
    public function createConfigFile()
    {
        ...
        $additionalConfigPart = $this->isNewerThan220()
            ? 'binlog_path = #'
            : 'max_matches = 1000';
        $connectionString = $this->isNewerThan220()
            ? 'listen = %s:%d'
            : 'address = %s' . PHP_EOL . 'port = %d';
        $connectionString = sprintf($connectionString, $sphinxConfig['server_addr'], $sphinxConfig['server_port']);
...
FILE;
    ...
    public function startSearchd()
    {
        if ($this->checkSearchdState() === false) {
            ...
            if ($exec['status'] === 0) {
                $this->_getLogHelper()->log(
                    $this,
                    'Daemon for ' . $this->getConfigFileName() . ' has been started',
                    null,
                    $exec['data']
                );
            } else {
    ...
    public function stopSearchd($stopwait = false)
    {
        if ($this->checkSearchdState() === true) {
            ...
            if ($exec['status'] === 0) {
                $this->_getLogHelper()->log(
                    $this,
                    'Daemon for ' . $this->getConfigFileName() . ' has been stopped',
                    null,
                    $exec['data']
                );
    ...
    {
        $path = Mage::helper('awadvancedsearch/config')->getSphinxServerPath();
        $cmd = $path . self::SEARCHD_CALL . ' -h';

        $helpCommandResult = $this->_execCommand($cmd);
        if (preg_match('/^[\D]*([0-9.]*).*/', $helpCommandResult['data'], $_m)) {
            return $_m[1];
        }
    }

    public function isNewerThan220()
    {
        return (version_compare($this->getVersion(), '2.2.0') >= 0);
    }
}
