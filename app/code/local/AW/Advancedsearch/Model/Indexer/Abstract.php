...
abstract class AW_Advancedsearch_Model_Indexer_Abstract
{
    ...
    protected function _removeTable()
    {
        $queryString = sprintf(
            ...
            $this->_getConnection()->quoteIdentifier($this->_getTableName())
        );
    ...
    public function reindex()
    {
        if ($this->getIndex() && $this->canIndex()) {
            $this->_getLogHelper()->log(
                $this,
                'Started building of catalog index table for index #' . $this->getIndex()->getId()
            );
            $this->_removeTable();
        ...
        if ($this->_isTableExists()) {
            ...
            try {
                ...
                if (AW_Advancedsearch_Helper_Data::DEBUG_MODE) {
                    $this->_getLogHelper()->log(
                        $this,
                        'Products: ' . $collection->getSize() . ', pages: ' . $pagesCount
                    );
                }
    ...
    public function getIndexTableModel()
    {
        if ($this->_indexTableModel === null && $this->_isTableExists()) {
            $this->_indexTableModel = new AW_Advancedsearch_Model_Zend_Db_Table(
                array(
    ...
    public function updateData($id)
    {
        if ($this->_isTableExists()) {
            $collection = $this->_prepareCollection();
            $searchKey = $this->_getPrimary();
            $field = ((strlen($this->_getPrimaryTable()) > 0) ? $this->_getPrimaryTable() . '.' : '')
                . $searchKey
            ;
