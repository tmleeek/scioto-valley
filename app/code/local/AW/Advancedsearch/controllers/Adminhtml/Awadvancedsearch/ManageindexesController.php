...
class AW_Advancedsearch_Adminhtml_Awadvancedsearch_ManageindexesController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function editAction()
    {
        ...
        $this->_initAction();
        if (!$this->getRequest()->getParam('fswe')
            || !Mage::helper('awadvancedsearch/forms')->getFormData($this->getRequest()->getParam('id'))) {
    ...
    protected function typeformAction()
    {
        ...
        if (($typeId = $this->getRequest()->getParam('typeId'))) {
            ...
            if ($typeFieldset) {
                ...
                $result['fieldset'] = Mage::getSingleton('core/layout')
                    ->createBlock('awadvancedsearch/adminhtml_indexes_edit_fieldset_error')
                    ->toHtml()
                ;
            }
        }
    ...
    protected function saveAction()
    {
        ...
        if ($request->getParam('status') !== null) {
            ...
            if ($request->getParam('type') && $request->getParam('type') !== null) {
                ...
                if (!in_array(0, $allStores)
                    && !array_intersect($allStores, $data['store'])
                    && (!$allStores || !in_array(0, $data['store']))
                ) {
                    $data['attributes'] = array();
                    if (is_array($request->getParam('attributes'))) {
                        $data['attributes'] = $request->getParam('attributes');
                    }
                    ...
                    $this->_getSession()->addError(
                        $this->__(
                            'Index with the same type has been already created for at least that one of selected stores'
                        )
                    );
                }
            } else {
