...
class AW_Autorelated_Model_Blocks extends Mage_Core_Model_Abstract
{
    ...
    protected function _beforeSave()
    {
        ...
        if ($this->getData('date_to') && !is_empty_date($this->getData('date_to'))) {
            $date = date(
                AW_Autorelated_Model_Mysql4_Blocks::MYSQL_DATETIME_FORMAT,
                strtotime($this->getData('date_to'))
            );
            $this->setData('date_to', $date);
        } else {
        ...
        if ($this->getData('date_from') && !is_empty_date($this->getData('date_from'))) {
            $date = date(
                AW_Autorelated_Model_Mysql4_Blocks::MYSQL_DATETIME_FORMAT,
                strtotime($this->getData('date_from'))
            );
            $this->setData('date_from', $date);
        } else {
