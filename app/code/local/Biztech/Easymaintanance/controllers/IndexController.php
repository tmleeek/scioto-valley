    ...
    class Biztech_Easymaintanance_IndexController extends Mage_Core_Controller_Front_Action
    {
        ...
        public function postnotifyAction(){
            ...
            if($post){
                ...
                $sql = "select notification_id from easysite_notify where email='".$notifyemail."'";
                $query = $read->query($sql);
                $result = $query->fetch();

                if($result){
                    $var1["result"]="error";
                    $var1["message"]='A user with same email address has already registered for Notification.';
                    ...
                    try{
                        ...
                        $fields = array();
                        $fields['name']= $notifyname;
                        $fields['email']= $notifyemail;
                        $connection->insert('easysite_notify', $fields);
                        $connection->commit();

                        $var1["result"]="success";
                        $var1["message"]='You are registered successfully for Notification.';
                        $notifydata=json_encode($var1);
                        $this->getResponse()->setBody($notifydata);
                        return;
                    }
