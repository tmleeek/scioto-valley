 ...
 * OrderController.php
 */
class Watsons_Exportorders_Adminhtml_OrderController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Backup list action
     */
    ...
    public function gridAction()
    {
        $this->getResponse()->setBody($this->getLayout()->createBlock(
            'exportorders/adminhtml_order_grid')->toHtml()
        );
    }
    ...
    public function createAction()
    {
        try {
            ...
            $last_run   = (int) Mage::getConfig()->getNode(
                Watsons_Exportorders_Model_Sync::EXPORT_ORDERS_LAST_RUN_PATH
                , Watsons_Exportorders_Model_Sync::SCOPE
            );
    ...
    {
        $order = Mage::getModel('exportorders/order')
    ...
    {
        try {
            $backup = Mage::getModel('exportorders/order')
                ->setName($this->getRequest()->getParam('name'))
                ->deleteFile();
        ...
        if (!is_array($orderNames)) {
            ...
            try {
                foreach ($orderNames as $orderName) {
                    $order = Mage::getSingleton('exportorders/order')->load($orderName);
                    $order->deleteFile();
                }
                $this->_getSession()->addSuccess(
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('exportorders/adminhtml/order');
    }
