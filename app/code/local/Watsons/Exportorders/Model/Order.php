 ...
 * Order.php
 */
class Watsons_Exportorders_Model_Order extends Varien_Object
{
    /* internal constants */
    ...
    public function setFile(&$content)
    {
        if (!$this->hasData('time') || !$this->hasData('type') || !$this->hasData('path')) {
            Mage::throwException(Mage::helper('exportorders')->__('Wrong order of creation for new sync'));
        }
    ...
    public function &getFile()
    {
        ...
        if (!$this->exists()) {
            Mage::throwException(Mage::helper('exportorders')->__(
    ...
    {
        if (!$this->exists()) {
            Mage::throwException(Mage::helper('exportorders')->__("Order file doesn't exist"));
        }
    ...
    public function open($write = false)
    {
        if (is_null($this->getPath())) {
            Mage::exception('Watsons_Sync', Mage::helper('exportorders')->__('Order file path don\'t specify'));
        }
        ...
        if (!$write && !$ioAdapter->fileExists($filePath)) {
            Mage::exception('Watsons_Sync', Mage::helper('exportorders')->__('Order file "%s" doesn\'t exist', $this->getFileName()));
        }
        ...
        try {
            ...
            Mage::exception('Watsons_Sync', Mage::helper('exportorders')->__('Order file "%s" can\'t read or write', $this->getFileName()));
        }
    ...
    public function read($length)
    {
        if (is_null($this->_handler)) {
            Mage::exception('Watsons_Sync', Mage::helper('exportorders')->__('Order file handler don\'t specify'));
        }
    ...
    public function write($string)
    {
        ...
        try {
            ...
            Mage::exception('Watsons_Sync', Mage::helper('exportorders')->__('Error write to Order file "%s"', $this->getFileName()));
        }
