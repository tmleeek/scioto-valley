...
RevLayers.prototype = {
    ...
    index: 0,
    layers: {},
    layerParams: 'align|style|text|left|top|max_width|max_height|whitespace|scaleX|scaleY|proportional_scale|animation|easing|speed|split|splitdelay|endanimation|endeasing|endspeed|endsplit|endsplitdelay|endtime|link_enable|link_type|link|link_open_in|link_slide|corner_left|corner_right|resizeme|hiddenunder|id|classes|title|rel|alt'.split('|'),
    ...
    setInAnimation: function(){
        ...
        TweenLite.killTweensOf(target, false);
        TweenLite.killTweensOf(animobject, false);
        TweenLite.set(target, {clearProps: 'transform'});
    ...
    selectLayer: function(){
        ...
        if (layer){
            ...
            this.layerParams.each(function(param){
                ...
                if (param == 'link_enable' || param == 'link_type' || param == 'animation' || param == 'endanimation'){
                    if (fireEvent){
                        setTimeout(function(){
            ...
            if (this.slider){
                var timeElm = this.list.down('#item_' + layer.params.serial).down('input.time'),
                    endtimeElm = $('layer_endtime');
                ...
                this.slider.on('slide', function (ev, ui) {
                    this.selectedLayer.params.time = Math.round(ui.values[0]);
                    this.selectedLayer.params.endtime = Math.round(ui.values[1]);
                    if (timeElm) timeElm.value = Math.round(ui.values[0]);
                    if (endtimeElm) endtimeElm.value = Math.round(ui.values[1]);
                }.bind(this));
            }
    ...
    updateList: function(){
        var slider = jQuery('#timeline');
        if (!slider.length) return;
        slider.find('span.min').html('0ms');
        slider.find('span.max').html(this.delay + 'ms');
        var max = this.delay,
            min = 0;
    ...
    getListItemName: function(layer){
        var cleanName = layer.text.stripTags().escapeHTML();

        switch (layer.video_type){
            case 'youtube':
                return 'Youtube: ' + cleanName;
                break;
            case 'vimeo':
                return 'Vimeo: ' + cleanName;
                break;
            case 'html5':
                return 'Video: ' + cleanName;
                break;
            default:
                return cleanName;
                break;
