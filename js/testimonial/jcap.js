...
function getBaseURL() {
    ...
    var baseURL = url.substring(0, url.indexOf('testimonial'));//tach chuoi link tu dau toi vi tri co chu 'testimonial'

    if(!baseURL.substring(0,url.indexOf('index.php'))){ //neu trong chuoi ko co index.php thi giu nguyen
	 /*   if (baseURL.indexOf('http://localhost') != -1) {
	        ...
	        return baseURL + "/";
	    //}
    } else {//neu trong chuoi co chu index.php thi loai
    	return baseURL.substring(0,url.indexOf('index.php'))+"/";
    }

}
;

var imgdir = getBaseURL() + "skin/frontend/default/default/images/cimg/";  // identify directory where captcha images are locatedvar jfldid = "security_code"; // identify word field id name
//alert(imgdir);
var jfldid = "security_code"; // identify word field id name
...
var security_code = hex_md5(document.getElementById(jfldid).value);

if (security_code==cword[anum-1]) {
return true;
}
