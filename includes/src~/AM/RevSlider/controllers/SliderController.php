...
class AM_RevSlider_SliderController extends Mage_Adminhtml_Controller_Action{
    ...
    protected function _processTxtImport($filePath){
    ...
    protected function _processZipImport($filePath){
        ...
        $sliderData = preg_replace_callback('/s:(\d+):"(.*?)";/', function($match){
        ...
        if (isset($sliderArray['params'])) {
            ...
            if (isset($sliderArray['params']['background_image'])){
                $sliderArray['params']['background_image'] = $this->_processImageZipImport($filePath, $sliderArray['params']['background_image']);
            }
            ...
            if (!isset($sliderArray['params']['hide_thumbs_delay_mobile']) || $sliderArray['params']['hide_thumbs_delay_mobile'] == ''){
        ...
        if ($sliderData !== false){
            ...
            if (is_array($cssArray)){
                foreach ($cssArray as $cssRule => $cssProperties){
                    ...
                    $cssModel = Mage::getModel('revslider/css')->loadByRule($handle);
                    $cssModel->setData('handle', $handle);
                    if ($handle != $cssRule) {
                        $cssModel->setData('hover', Mage::helper('core')->jsonEncode($cssProperties));
                    }else {
                        $cssModel->setData('params', Mage::helper('core')->jsonEncode($cssProperties));
                    }

                    $cssModel->save();
                    //Mage::log($cssModel->getData());
                }
            }
        }
        ...
        if ($animationData !== false){
            ...
            if ($animationArray !== false && is_array($animationArray)){
                foreach ($animationArray as $animation){
                    ...
                    $animationModel = Mage::getModel('revslider/animation')->loadByName($animation['handle']);
                    if (!$animationModel->getId()) {
                        $animationModel->setData('name', $animation['handle']);
                    }
                    $animationModel->setData('params', Mage::helper('core')->jsonEncode($animation['params']));

                    $animationModel->save();
                    //Mage::log($animationModel->getData());
        ...
        //if (isset($sliderArray['slides'])){
            foreach ($sliderArray['slides'] as $slideArray){
                if (!isset($slideArray['params'])) continue;

                if (isset($slideArray['params']['image_url'])){
                    $slideArray['params']['image_url'] = $this->_processImageZipImport($filePath, $slideArray['params']['image_url']);
                }

                if (isset($slideArray['params']['image'])){
                    $slideArray['params']['image_url'] = $this->_processImageZipImport($filePath, $slideArray['params']['image']);
                ...
                foreach ($slideArray['layers'] as $i => $layer){
                    ...
                    if (isset($layer['image_url'])){
                        $slideArray['layers'][$i]['image_url'] = $this->_processImageZipImport($filePath, $layer['image_url']);
                    }
                    ...
                    if (isset($layer['video_image_url'])){
                        $slideArray['layers'][$i]['video_image_url'] = $this->_processImageZipImport($filePath, $layer['video_image_url']);
                    }
                }
    ...
    public function importPostAction(){
        if ($this->getRequest()->isPost()){
            try{
                ...
                $uploader->setAllowedExtensions(array('txt', 'zip'));
                ...
                switch ($fileInfo['extension']){
                    case 'txt':
                        $slider = $this->_processTxtImport($filePath);
                        break;
                    case 'zip':
                        $slider = $this->_processZipImport($filePath);
                        break;
                    default:
                        throw new Exception(Mage::helper('revslider')->__('File not supported.'));
                }
                if ($slider->getId()) {
                    $this->_getSession()->addSuccess(Mage::helper('revslider')->__(
                        ...
                        Mage::helper('revslider')->getFrontendUrl('revslider/index/preview', array('id' => $slider->getId()))
                    ));
                }
            }catch (Exception $e){
                $this->_getSession()->addError($e->getMessage());
    ...
    public function massDeleteAction(){
        ...
        if (count($ids)){
            ...
            if ($i){
                $this->_getSession()->addSuccess(
                    Mage::helper('revslider')->__('%d sliders deleted sucessfully', $i)
                );
            }
        }
