...
class Watsons_Sync_Model_Sync extends Mage_Core_Model_Abstract
{
    ...
    public function exportOrders($last_run=null, $state=null) {
        ...
        if ($state != null) {
	    Mage::log("Exporting orders cond1", Zend_Log::DEBUG);
            $orderCollection->addFieldToFilter('state', $state);
        } else {
	    Mage::log("Exporting orders cond2", Zend_Log::DEBUG);
	...
	Mage::log("Exporting orders last_run not null", Zend_Log::DEBUG);
        }
...
// exit;
		...
		Mage::log("Exporting orders got orders", Zend_Log::DEBUG);
            $customerModel->reset();
		...
		Mage::log("Exporting orders customerid ", Zend_Log::DEBUG);
            }
		...
		Mage::log("Exporting orders has invoice", Zend_Log::DEBUG);
		...
		Mage::log("Exporting orders item loop", Zend_Log::DEBUG);
                $line = array(
                    ...
                    , number_format($item->getPrice(), self::NUM_DECIMALS,'.','')
